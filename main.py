...
def run_model(model_path, algo, episodes=5):
    print(f"Loading {algo} model from {model_path}...")
    
    # 12x12 Grid needs more steps
    env = DroneScoutingEnv(render_mode="human", seed=200, max_steps=1000)
    
    model = None
    policy_net = None
    
    if algo == "ppo": model = PPO.load(model_path)
    elif algo == "dqn": model = DQN.load(model_path)
    ...
    for ep in range(episodes):
        ...
        while not done:
            ...
            if term or trunc:
                ...
                print(f"End: {outcome}. Reward: {total_reward:.2f}")
...
if __name__ == "__main__":
    ...
    if args.mode == "random":
        ...
        if not args.path: print("Error: Provide --path")
        else: run_model(args.path, args.algo)